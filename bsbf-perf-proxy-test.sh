...
if [ $# -ne 2 ]; then
	echo "Usage: $0 <fw-u32|fw-flower|u32-u32|u32-flower|no-matching|disable-gso-gro|default> <sing-box-tun|sing-box-tproxy|v2ray-tproxy>"
...
ip netns exec server sing-box run -c proxy-files/sing-box-server.json &
if [ "$PROXY_PROGRAMME" = "sing-box-tun" ]; then
ip netns exec client sing-box run -c proxy-files/sing-box-client-tun.json &
elif [ "$PROXY_PROGRAMME" = "sing-box-tproxy" ]; then
ip netns exec client sing-box run -c proxy-files/sing-box-client-tproxy.json &
elif [ "$PROXY_PROGRAMME" = "v2ray-tproxy" ]; then
ip netns exec client v2ray run -c proxy-files/v2ray-client-tproxy.json &
fi

# Load the nftables rules and configure policy routing for tproxy.
if [ "$PROXY_PROGRAMME" = "sing-box-tproxy" ] || [ "$PROXY_PROGRAMME" = "v2ray-tproxy" ]; then
ip netns exec client nft -f proxy-files/bsbf-perf-proxy-test.nft
...
elif [ "$PROXY_PROGRAMME" = "sing-box-tun" ]; then
